...
\item \textbf{Least restrictive control}: A lower-priority vehicle assumes that higher-priority vehicles will arrive at their targets on time, but has no other information.
\item \textbf{Robust trajectory tracking}: Each vehicle declares a nominal trajectory which can be robustly tracked under disturbances.
\end{itemize}
...
The latest departure time $\ldt_i$ is then given by $\arg \inf_t x_i(\edt) \in \brs_i(t)$.
