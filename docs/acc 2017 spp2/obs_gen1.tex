...
Disturbances and incomplete information significantly complicate the SPP scheme. The main differences are below:
...
We present three methods for address the above issues. Each method has its advantages and disadvantages depending on the situation. The three methods are as follows:\MCnote{Bold name of each method?}
\begin{itemize}
...
\begin{equation}
\label{eq:opt_ctrl_i}
u_i^*(t,x_i) =  \arg \min_{u_i \in \cset} \max_{d_i \in \dset} p \cdot f_i(t, x_i, u_i, d_i)
\end{equation}
...
&x_j(\cdot) \text{ satisfies \eqref{eq:dyn_cc} with } x_j(\ldt) = x_{j0}, (x_j(t) = y\}
\end{aligned}
\end{equation}
...
\end{equation}
\noindent where $\bar l$ is chosen to be\footnote{In practice, we define the target set to be a small region around the vehicle's initial state.} such that $\bar l (y) = 0 \Leftrightarrow y = x_j(\ldt)$.
