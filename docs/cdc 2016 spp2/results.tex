...
|d_{x,i}| & \le \bar{d_{x}}, |d_{y,i}| \le \bar{d_{y}}, |d_{\theta,i}| \le \bar{d_{\theta}}
\end{aligned}
\end{equation}

\noindent where $p_i = (p_{x,i}, p_{y,i})$ represent vehicle $\veh_i$'s position, $\theta_i$ represents $\veh_i$'s heading, and $d = (d_{x,i}, d_{y,i}, d_{\theta,i})$ represent the disturbance in the three states. Disturbance in each state is bounded from top and below. The control of $\veh_i$ is $u_i = (v_i, \omega_i)$, where $v_i$ is the speed of $\veh_i$ and $\omega_i$ is the turn rate; both controls have a lower and upper bound. For illustration purposes, we chose $\underline{v} = 0.5, \bar{v} = 1, \bar\omega = 1$; however, our method can easily handle the case in which these inputs differ across vehicles. The disturbance bounds are chosen as $\bar{d_{x}} = \bar{d_{y}} = 0.1$ and $\bar{d_{\theta}} = 0.2$, which correspond to a 10\% disturbance. The optimal control for vehicle $i$ can be obtained by optimizing the associated Hamiltonian, $H_i(t, D_{\bm{x}_i} V_i(\bm{x}_i,t), V_i(\bm{x}_i,t))$, and is given by:
...
\subsection{Centralized Controller}
\begin{itemize}
\item Advantages: Optimal control at all times, so smallest possible TTR \JFFnote{Not true! Larger induced obstacle can lead to worse TTR in low-priority vehicles} for each vehicle for a given priority order.
\item Limitations: Need a centralized planner to ensure that the optimal control is being applied at all times; no flexibility in control.
\end{itemize}

\subsection{Least Restrictive Control}
\begin{itemize}
\item Advantages: Greater control flexibility (as long as the vehicle operates within the pre-designated safety bounds)
\item Limitations: Big obstacles so TTR increases significantly
\end{itemize}

\subsection{Robust Trajectory Tracking}
\begin{itemize}
\item Advantages: Practical (easy to implement); greater control flexibility (as long as the vehicle operates within the pre-designated bubble); good trade-off between TTR and "altruism" (more suitable for scenarios where there is no strong sense of priority).
\item Limitations: limited to the system which can be transformed into relative dynamics, TTR can increase  significantly, currently heuristics are used to find the appropriate bubble size.
\end{itemize}