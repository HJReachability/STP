...
\SBnote{I think in the equation above, $u_j$ should be $u_j(t, x_j)$ and $u\in \partial \brs_i(t)$ should be $x_j(t) \in \partial \brs_i(t)$.}

Such a controller allows each higher priority vehicle to use any controller it desires, except when it is on the boundary of the BRS, $\partial \brs_i$, in which case the optimal control $u_j^*(x_j)$ \SBnote{I think it should be $u_j^*(t, x_j)$ because obstacles are time varying.} given by \eqref{eq:opt_ctrl_i} must be used to get to the target on time. This assumption is the weakest assumption that could be made by lower priority vehicles given that the higher priority vehicles will get to their targets on time.
...
\SBnote{Again, let'd use the conventional trajectory based definition of FRS in the equation below.}
\begin{equation}
\label{eq:frs2}
\begin{aligned}
