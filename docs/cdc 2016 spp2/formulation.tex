...
In this paper we impose a mild structure to the general problem in order to trade complexity for optimality: we assign a priority to each vehicle. \JFFnote{Careful! Not a contribution of this paper, should rephrase.} While traveling to its target set, a vehicle may ignore the presence of lower priority vehicles, but must take full responsibility for avoiding higher priority vehicles. Such a joint path planning scheme makes intuitive and practical sense\JFFnote{weak argument (+ completely unjustified), should cite NASA and/or other work}, and the priorities can be assigned, for example, on a first-come-first-served basis.
