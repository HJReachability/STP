...
Consider $N$ vehicles \SBnote{We are using $Q_i$ to denopte vehicles in Fig 1 so we should introduce the vehicle notation before that.} whose joint dynamics described by the time-varying ordinary differential equation (ODE)
...
\SBnote{Shouldn't we add the time index for $\pos_i$ as well in this equation?}
\begin{equation}
