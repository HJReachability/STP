...
\SBnote{Let's write the traditional trajectory based definition of FRS. Also, the definition below is wrong as we discussed earlier. It should be there exist a disturbance and not for all disturbance.}
\begin{equation}
\label{eq:frs}
\begin{aligned}
...
\SBnote{It'd be helpful to write that the computation is done recursively and that there are no obstacles for $Q_1$. Currently, it's not clear how one will get the optimal control, as it seems a circular problem.}
...
\noindent where $l$ is chosen to be such that $\targetset = \{x_j(t_0)\}$. \SBnote{The next line is not required; it will unnecessarily confuse the reader.} In practice, when there is uncertainty in the initial state of $\veh_j$, we set $\targetset_j$ to be a small region around $x_j(t_0)$. To impose that the optimal control is used, we substitute $u_j = u_j^*(t, x_j)$ into \eqref{eq:FRS_j} instead of minimizing over $u_j$. \SBnote{Why don't we qrite HJVI equations above with the optimal control plugged in? This is a bit confusing and indirect way to explain the same thing.}
