...
Conveniently, $\frs_j(t)$ can also be computed using $\eqref{eq:FRS_i}$ with $l$ chosen to be such that $\targetset = \{x_j(t_0)\}$. In practice, when there is uncertainty in the initial state of $\veh_j$, we set $\targetset$ to be a small region around $x_j(t_0)$. Note that \eqref{eq:FRS_i} is just a simplified version of \eqref{eq:HJIVI_i}

\begin{equation}
\label{eq:FRS_i}
\begin{aligned}
&D_t V_i(t, x_i) + H_i\left(t, x_i, D_{x_i} V\right) = 0, t \in [\edt, \sta]\\
&V_i(\sta, x_i) =l_i(x_i) \\
