...
%%%
\begin{abstract}
Recently, there has been an immense surge of interest in using unmanned aerial vehicles (UAVs) for civil purposes. Multi-UAV systems are safety-critical systems, and safety guarantees must be made to ensure no undesirable configurations such as collisions occur. Hamilton-Jacobi (HJ) reachability is the ideal tool for analyzing such safety-critical systems because it provides safety guarantees and is flexible in terms of system dynamics; however, its direct application is limited to small-scale systems of no more than two vehicles because of the exponential-scaling computation complexity. By assigning vehicle priorities and allowing higher priority vehicles to ignore lower priority vehicles, the sequential path planning (SPP) method allows multi-vehicle path planning to done with a computation complexity that scales linearly with the number of vehicles. Previously the SPP method assumed that the dynamics of all vehicles have no disturbances, and that every vehicle has perfect knowledge of the position of higher-priority vehicles. In this paper, we make SPP more practical by providing three different methods for disturbances in dynamics and imperfect knowledge of higher-priority vehicles. We demonstrate our proposed methods in several simulations.
\end{abstract}
