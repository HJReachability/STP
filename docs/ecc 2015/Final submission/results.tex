...
%
%\section{Results: Two Vehicles with Kinematics Model \label{sec:2vek}}
...
%\end{equation}
%
%\noindent where $v_1=v_2=1$ are the maximum speeds of the vehicles and $\ctrlin$ is the unit disk. Under this model, each vehicle can move in any direction at some maximum speed. With the above dynamics, we can optimize Hamiltonian $\ham_i(t, D_{\x_i}\soln_i(\x_i,t), \soln_i(\x_i,t))$ for each vehicle to obtain the optimal control
%%
%%\bq
...
%\bq
%\x_1^0 = (-0.5, 0), \x_2^0 = (0.5, 0), \quad \tf_1 = \tf_2 = 0
%\eq
%
...
%\begin{figure}
%	\centering
%	\includegraphics[width=0.175\textwidth]{"fig/kin_ic"}
...
%\begin{figure}
%	\centering
%	\includegraphics[width=0.275\textwidth]{"fig/kin_reach"}
...
%\begin{figure}
%	\centering
%	\includegraphics[width=0.35\textwidth]{"fig/kin_result"}
...
%
%Computations were done on a $200^2$ grid. Trajectory planning for vehicle 1 took approximately 0.34 seconds using the fast marching method \cite{sethian96}. Trajectory planning via solving Equation (\ref{eq:HJI}) for vehicle $2$ given the trajectory for vehicle 1 took approximately 25 seconds using a modified implementation of \cite{LSToolbox}. Computations were done on a Lenovo T420s laptop with a Core i7-2640M processor, and are orders of magnitude faster than doing a 4D HJI calculation, which took approximately 30 minutes.
%
...
\section{Results: Four Vehicles with Constrained Turn Rate}
...
The target sets $\target_i$ of the vehicles are all 4 circles of radius $0.1$ in the domain. The centers of the target sets are at $(0.7, 0.2), (-0.7, 0.2), (0.7, -0.7), (-0.7, -0.7)$ for vehicles $i=1,2,3,4$, respectively. The obstacles are rectangles near the middle of the domain. %The system's initial conditions and domain are shown in Figure \ref{fig:kin_ic}.The domain $\amb$ and obstacles $\obs$ are the same as those of the example in Section \ref{sec:2vek}. 
