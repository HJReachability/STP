...
\SetKwInOut{Output}{output}
	\DontPrintSemicolon
	\caption{STP algorithm in the absence of disturbance and intruders}
	\label{alg:basic}
	\Input{Set of vehicles $\veh_i$ in the descending priority order, their dynamics \eqref{eq:dyn_no_dstb} and initial states $\state_i^0$;\newline
	Vehicle destinations $\targetset_i$ and static obstacles $\soset_i$.}
    \Output{Provably safe vehicle trajectories to respective destinations and goal-satisfaction controller $\ctrl^\text{basic}(\cdot)$ for all vehicles.}
	\For{\text{$i=1:N$}}{
