...
Note that in this work, the reduced control authority $\cset^p\subset\cset$ and the parameter $R_{\text{EB}}$ are selected by trial and error such that the tracking error bound $\disckernel_j$ is nonempty. Nevertheless, selecting these parameters in a principled way is an interesting future direction.
