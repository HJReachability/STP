...
\begin{assumption}
\label{as:avoidOnce}
At most one intruder affects the SPP vehicles at any given time. The intruder exits the altitude level affecting the SPP vehicles after a duration of $\iat$. \SBnote{Distinction between time at which intruder appears and time at which avoidance is performed. ($\underline{t}$ vs. $t^*$)}
\end{assumption}
