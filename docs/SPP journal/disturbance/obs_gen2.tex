...
\item Compute the BRS $\brs_i^\text{dstb}(t, \sta_i)$ defined in \eqref{eq:BRS}. The latest departure time $\ldt_i$ is then given by $\arg \sup_t \state_{i0} \in \brs_i^\text{dstb}(t, \sta_i)$.
\item Compute the FRS $\frs_i^\text{lrc}(\ldt_i, t)$ in \eqref{eq:FRS2}. Given $\frs_i^\text{lrc}(\ldt_i, t)$ and $\brs_i^\text{dstb}(t, \sta_i)$, compute the positions that the $\veh_i$ could be in. The set of these positions is given by \eqref{eqn:lrcObs2}.
