...
\SBnote{Notation in this section is different from the rest of the paper. For example, $x$ is used for the state instead of $z$. An easy fix will be to define $x$ as the state in the beginning instead of $z$. Also, FRS and BRS are defined again in this section; we can simply use the concise notation introduced earlier.}
...
\SBnote{Summary algorithms with references to basic SPP algorithm for each method}
