...
\SBnote{For RTT method, we never explained where the induced obstacles will be used! In general, we need to clearl;y explain the BRS that we are computing for computing the error bound and nominal trajectory using equations. Especially for the nominal trajectory part, we should explain that we are using dynamics without discturbance, target set is reduced, obstacles are induced obstacles and control authority is reduced.} 
...
\item Compute a nominal trajectory. The latest departure time $\ldt_i$ is then given by $\arg \sup_t \state^0_i \in \brs_i(t)$.
