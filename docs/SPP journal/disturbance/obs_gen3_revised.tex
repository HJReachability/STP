...
Finally, given $\errorbound_i$, we can guarantee that $\veh_i$ will reach its target $\targetset_i$ if $\errorbound_i \subseteq \targetset_i$; thus, in the path planning phase, we modify $\targetset_i$ to be $\tilde{\targetset}_i := \{\state_i: \errorbound_i + \state_i \subseteq \targetset_i\}$, and compute a BRS, with the control authority $\cset^\pos_i$, that contains the initial state of the vehicle. Mathematically,
