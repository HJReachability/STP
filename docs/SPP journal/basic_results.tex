...
\item FRS and BRS definitions may still not be correct. We should use $y$ as the set variable instead of state. Otherwise, there will be a confusion between state and trajectory. 
\item Make sure that we are following a consistent superscript and subscript structure to denotes our BRSs and FRSs.
\item Make sure that we define $\sta_i$ in Basic SPP section before presenting the algorithm. 
\item Check all the algorithms after everything else is done.
\end{itemize}}
...
\begin{equation}
\label{eq:target_sim}
\targetset_i = \{\state_i: \|p_i - c_i\|_2 \le r\}
\end{equation}
