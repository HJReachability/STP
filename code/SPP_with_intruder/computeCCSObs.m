...
% [data, tau] = computeBaseObs( ...
%   data0, tau, schemeData, minWith, extraargs)
%
% Computes base obstacles with the initial obstacle data0, at times tau, 
% and with parameters schemeData and obstacles. This function is more of a
% hack right now, but can be improved later on.
%
% Inputs:
...
%                inequality)
%   genparams  - obstacle generation parameters 
...
if isfield(extraArgs, 'genparams')
  genScheme = 'centralized';
  gen_tau = genparams.tau;
  gen_data = genparams.data;
  gen_resetR = genparams.reset_thresholds;
else
...
for i = 2:length(tau)
  ...
  if strcmp(genScheme, 'centralized')
    % Set the schemeData so that we pass on appropriate data to generate
    % optimal control as well as threshold.
    data_BRS = eval(get_dataStr(g.dim, 'i-1', 'gen_data')); 
    schemeData.data_BRS = data_BRS;
    % Find if we need to add any threshold to the optimal control
    schemeData.reset_input = false;
    [y0, update_flag] = check_obssize(g, y0, gen_resetR);
    if update_flag
      schemeData.reset_input = true;
    end
  else
    error('No other scheme has been implemented yet!')
  end
