...
for i=1:vnum
    ...
    [~,h{i,1}] = contour(g2D.xs{1},g2D.xs{2}, data2D, [0 0], 'color', allVehicles{i}.fig_color,'linewidth',2);
    % State
    plot(allVehicles{i}.x_nom(1,t), allVehicles{i}.x_nom(2,t), 'marker', '*', 'color', allVehicles{i}.fig_color,'markersize',5);
    % Arrow for the state
    dirn = 0.2*[cos(allVehicles{i}.x_nom(3,t)) sin(allVehicles{i}.x_nom(3,t))];
    h{i,2} = quiver(allVehicles{i}.x_nom(1,t), allVehicles{i}.x_nom(2,t), dirn(1), dirn(2), ...
        'maxheadsize', 50, 'marker', '*', 'color', allVehicles{i}.fig_color);
    % Bubble around state
    ...
    h{i,4} = plot(allVehicles{i}.x_nom(1,:), allVehicles{i}.x_nom(2,:), 'color', allVehicles{i}.fig_color,'linestyle', ':');
    drawnow;
end
