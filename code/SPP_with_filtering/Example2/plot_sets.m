...
index = 30;
figure,
[g2D, data2D] = proj2D(g, allVehicles{4}.reach(:,:,:,index+1), [0 0 1], 5*pi/4);
[~,h1] = contour(g2D.xs{1},g2D.xs{2}, data2D, [0 0], 'color', 'r', 'linestyle', '--');
hold on;
...
index = 0;
figure,
[g2D, data2D] = proj2D(g, allVehicles{1}.collisionmat(:,:,:,index+1), [0 0 1]);
[~,h1] = contour(g2D.xs{1},g2D.xs{2}, data2D, [0 0], 'color', 'r', 'linestyle', '--');
hold on;
[g2D, data2D] = proj2D(g, allVehicles{1}.reach(:,:,:,index+1), [0 0 1]);
[~,h2] = contour(g2D.xs{1},g2D.xs{2}, data2D, [0 0], 'color', 'b', 'linestyle', '--');
