...
allVehicles{1}.x(:,1) = [ 0,  0.6, 3*pi/2]';
allVehicles{2}.x(:,1) = [ 0, -0.85, pi/2]';
% allVehicles{2}.x(:,1) = [ -0.04, -0.66, pi/2]';
% allVehicles{2}.x(:,1) = [ -0.06, -0.94, pi/2]'; -- still gives me one
% step improvement
% pi/3 is a promising heading

% Velocity of two vehicles
allVehicles{2}.velocity = 3;
allVehicles{2}.turnRate = 4;
...
%   allVehicles{i}.collisionmat(:,:,:,1:end-num_steps) = [];
   ...
   allVehicles{i}.mment(:,1:start_index+1) = repmat(temp_m, 1, int64(start_index+1));
