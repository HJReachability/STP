...
if (strcmp(vehicle.state_uncertainty, 'circular'))
    ...
    collisionObs = sqrt((g.xs{1} - mment(1)).^2 + (g.xs{2} - mment(2)).^2 + + (g.xs{3} - mment(3)).^2) - (mment_radius+capture_radius);
end
% Ellipsoidal model
if (strcmp(vehicle.state_uncertainty,'ellipsoid'))
    ...
    collisionObs = sqrt((1/(axis1_radius+capture_radius)^2)*(g.xs{1} - mment(1)).^2 + (1/(axis2_radius+capture_radius)^2)*(g.xs{2} - mment(2)).^2 +...
        (1/(axis3_radius+capture_radius)^2)*(g.xs{3} - mment(3)).^2) - 1;
end
