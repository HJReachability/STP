function vehicle = ComputeCollisionObs_wGridM(g,vehicle, flag)
...
schemeData.grid = g_new;
target = migrateGrid(g, vehicle.reach(:,:,:,end), g_new);
...
while(tMax - tNow > small * tMax)
    ...
    % Input
    schemeData.U = migrateGrid(g, vehicle.optU(:,:,:,end-reach_index+tindex+1), g_new);
    schemeData.V = migrateGrid(g, vehicle.optV(:,:,:,end-reach_index+tindex+1), g_new);
...
hamValue = vnom*deriv{1}.*cos(g.xs{3}) + d1*abs(deriv{1}) + vnom*deriv{2}.*sin(g.xs{3}) ...
    + d2*abs(deriv{2}) + 0.25*(deriv{1}.*cos(g.xs{3}) + deriv{2}.*sin(g.xs{3})) + ...
    w.*deriv{3} + d3*abs(deriv{3});
