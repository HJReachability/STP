...
while(tMax - tNow > small * tMax)
    ...
    % Input
    % Migrate reachable set and then compute optimal control
    reach_data = migrateGrid(g, vehicle.reach(:,:,:,end-reach_index + tindex), g_new);
...
hamValue = vnom*deriv{1}.*cos(g.xs{3}) + d1*abs(deriv{1}) + vnom*deriv{2}.*sin(g.xs{3}) ...
    + d2*abs(deriv{2}) + v.*(deriv{1}.*cos(g.xs{3}) + deriv{2}.*sin(g.xs{3})) + ...
