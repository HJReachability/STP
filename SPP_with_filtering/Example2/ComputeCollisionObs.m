...
while(tMax - tNow > small * tMax)
    ...
    % Input
    schemeData.U = vehicle.optU(:,:,:,end-reach_index + tindex);
    schemeData.V = vehicle.optV(:,:,:,end-reach_index + tindex);
...
hamValue = vnom*deriv{1}.*cos(g.xs{3}) + vnom*deriv{2}.*sin(g.xs{3}) ...
    + v.*(deriv{1}.*cos(g.xs{3}) + deriv{2}.*sin(g.xs{3})) + ...
    + d1 * sqrt(deriv{1}.^2 + deriv{2}.^2) + w.*deriv{3}...
    + d3*abs(deriv{3});
